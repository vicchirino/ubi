...
module.exports = {
  async delegateAndTest(fromAccount, toAddress, percentage, ubi) {
    ...
    await expect(ubi.connect(fromAccount).delegate(toAddress, percentage)).to.emit(ubi, "DelegateChange").withArgs(fromAccount.address, toAddress);
    return;
